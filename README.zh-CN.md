...
桌面应用版本来自仓库根目录的 `build.zig.zon`（当前为 `1.40.0`），也是
...
Ping 往返延迟（毫秒），每 5 秒刷新；命令右侧也会显示读数或开关状态，
标题栏太窄时可重新打开命令面板查看完整读数。
切换会话或分屏焦点后，会改为测量对应的 SSH 主机；再次执行命令即可停止。
`RTT …` 表示首次测量中，`RTT —` 表示无响应或无法探测。该功能测量直连主机的
ICMP 延迟，服务器禁用 Ping 或会话经过 `ProxyJump` 时无法提供数值。
开关仅对当前窗口生效，默认关闭；需要通过 WispTerm 启动的 SSH 会话元数据，
切换到本地会话时隐藏读数。
...
Xu, Z.-G. (2026). *WispTerm* (Version 1.40.0) [Computer software]. Zenodo.
