...
| `http-use-system-proxy`     | `false`    | Send AI provider HTTP through a proxy. Leave off to connect directly. |
| `http-proxy`                | *(empty)*  | Proxy used when `http-use-system-proxy` is on. Empty uses the system proxy (WinHTTP on Windows, the system session on macOS, `https_proxy` / `http_proxy` / `all_proxy` on Linux). Otherwise `host:port` or `http://host:port`, for example `127.0.0.1:6789`. |
